...
from backend.g4f.quora.api import retry_request
...
class TestRetryRequest(unittest.TestCase):
    ...
    def test_exponential_backoff(self):
        ...
        requests.get = MagicMock(
            side_effect=[requests.exceptions.RequestException] * 2 + [mock_response]
        )

        # Call the function and assert that it retries with exponential backoff
        with self.assertLogs() as logs:
            response = retry_request(
                requests.get, "http://example.com", max_attempts=3, delay=1
            )
            self.assertEqual(response.status_code, 200)
