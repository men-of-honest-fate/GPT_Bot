...
class Account:
    ...
    def create():
        ...
        f = r.text
        tid = re.search(
            '<input type="hidden" name="next_id" id="next_id" value="(\w+)">', f
        ).group(1)
        if len(tid) == 0:
...
class Completion:
    @staticmethod
    def create(account_data, prompt: str, message=False):
        param = {
        ...
        r = requests.get(
            "https://italygpt.it/question",
            headers=account_data._header,
            params=param,
            stream=True,
        )
