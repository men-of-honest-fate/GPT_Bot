...
class ChatCompletion:
    ...
    def create(
        ...
        res = requests.post(
            url=url, data=json.dumps(data), headers=headers, proxies=proxies
        )
    ...
    @staticmethod
    def randomStr():
        return "".join(random.choices(string.ascii_lowercase + string.digits, k=34))[
            :11
        ]

    @classmethod
    def __build_prompt(
        cls, context: str, message: list, isCasuallyFineTuned=False, last=15
    ):
        prompt = context + "\n\n" if context else ""
