...
response = post(
    "https://usjsmufuzdcrrceuhnyj.supabase.co/auth/v1/signup",
    headers=headers,
    json=json_data,
)
print(response.json())
...
supabase_auth_token = dumps(
    [access_token, refresh_token, None, None, None], separators=(",", ":")
)
print(supabase_auth_token)

cookies = {"supabase-auth-token": supabase_auth_token}
...
response = post(
    "https://openprompt.co/api/chat2", cookies=cookies, json=json_data, stream=True
)
