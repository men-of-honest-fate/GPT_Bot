...
class Completion:
    model = {"model": {"id": "gpt-3.5-turbo", "name": "Default (GPT-3.5)"}}

    def create(prompt: str = "hello world", messages: list = []) -> T3nsorResponse:
        response = post(
            "https://www.t3nsor.tech/api/chat",
            headers=headers,
            json=Completion.model | {"messages": messages, "key": "", "prompt": prompt},
        )
