...
from backend.g4f.quora import extract_formkey
from backend.g4f.quora.mail import Emailnator
...
class Account:
    ...
    def create(
        ...
        client.headers["poe-formkey"] = extract_formkey(
            client.get("https://poe.com/login").text
        )
        client.headers["poe-tchannel"] = client.get(
            "https://poe.com/api/settings"
        ).json()["tchannelData"]["channel"]
