...
from gpt4free import forefront, theb, you

from backend.g4f import quora
...
def query_forefront(question: str, proxy: Optional[str] = None) -> str:
    ...
    # get a response
    try:
        return forefront.Completion.create(
            token=token, prompt="hello world", model="gpt-4", proxy=proxy
        ).text
...
def query_quora(question: str, proxy: Optional[str] = None) -> str:
    ...
    return quora.Completion.create(
        model="gpt-4", prompt=question, token=token, proxy=proxy
    ).text
...
def query(
    user_input: str, selected_method: str = "Random", proxy: Optional[str] = None
) -> str:
    ...
    while not success and query_methods_list:
        ...
        chosen_query_name = [
            k for k, v in avail_query_methods.items() if v == chosen_query
        ][0]
        try:
