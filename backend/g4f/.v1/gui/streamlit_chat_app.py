...
from query_methods import avail_query_methods, query
...
if (
    ...
    or st.session_state["current_conversation"] is None
):
...
user_input = input_placeholder.text_input(
    ...
    key=f"input_text_-1",  # {st.session_state["input_field_key"]}
)
...
if (user_input and user_input != st.session_state["input_text"]) or submit_button:
    ...
    user_input = input_placeholder.text_input(
        "You:",
        value=st.session_state["input_text"],
        key=f'input_text_{st.session_state["input_field_key"]}',
...
if search_query:
    ...
    conversations = sorted(
        filtered_conversations,
        key=lambda x: Levenshtein.distance(search_query, x[1]["user_inputs"][0]),
    )
...
for idx, conversation in enumerate(conversations):
    if sidebar_col1.button(
        f"Conversation {idx + 1}: {conversation['user_inputs'][0]}",
...
if conversation_to_display["generated_responses"]:
    for i in range(len(conversation_to_display["generated_responses"]) - 1, -1, -1):
        message(
            conversation_to_display["generated_responses"][i],
            key=f"display_generated_{i}",
        )
        message(
            conversation_to_display["user_inputs"][i],
            is_user=True,
            key=f"display_user_{i}",
        )
